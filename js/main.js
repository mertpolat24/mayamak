...
window.MayamakApp = (function () {
  function initHeader() {
    ...
    function placeNav() {
      ...
      if (mq.matches) {
        ...
        closeNav();
        if (navHost && nav.parentElement !== navHost) {
  ...
  function initContactLinks() {
    ...
    if (addressEl && c.addressLines) {
      addressEl.innerHTML = c.addressLines.join("<br>");
    }

    var socialEl = document.getElementById("top-bar-social");
    if (socialEl && c.social) {
      function socialItem(url, label, svg) {
        if (url) {
          return (
            '<a href="' + url + '" class="social-link" target="_blank" rel="noopener noreferrer" aria-label="' + label + '">' +
              svg +
            '</a>'
          );
        }
      ...
      var fbSvg = '<svg viewBox="0 0 24 24"><path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/></svg>';
      ...
      var liSvg = '<svg viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.062 2.062 0 012.063-2.063 2.063 2.063 0 012.063 2.063 2.062 2.062 0 01-2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>';

      socialEl.innerHTML =
        socialItem(c.social.facebook, "Facebook", fbSvg) +
        socialItem(c.social.instagram, "Instagram", igSvg) +
        socialItem(c.social.linkedin, "LinkedIn", liSvg);
    }
  }
  ...
  function renderHomeReferences() {
    var setA = document.getElementById("home-references-set-a");
  ...
  function renderPrograms(container) {
    if (!container) return;
    var programs = window.MAYAMAK_DATA.programs;
  ...
  function setActiveNav() {
    ...
    document.querySelectorAll('.main-nav a[data-nav="companies"]').forEach(function (a) {
      if (page === "companies") a.classList.add("active");
    });
  }
